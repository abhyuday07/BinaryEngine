...
class Preprocessor(object):
    ...
    def is_source_root(self, path):
        ...
        for f in os.listdir(path):
            if f.lower() in ["install", "readme", "readme.md", "makefile", "configure", "config", "copying",
                             "cmakelist.txt", "changelog"]:
                count += 1
        return count

    def find_source_root(self):
        if self.is_cmake_pj(self.path) or self.is_make_pj(self.path) or self.is_autogen(
                self.path) or self.is_custom_make(self.path):
            return self.path
        ...
        for f in file_list:
            ...
            if f.lower() in ["src", "source"]:
                if self.is_cmake_pj(temp_path) or self.is_make_pj(temp_path) or self.is_autogen(
                        temp_path) or self.is_custom_make(temp_path):
                    return temp_path
    ...
    def __extract_subprocess(self, commandline):
        command_list = []
                ...
                with open(info_file, "r") as f:
                    for line in f.readlines():
                        if re.search('\\b' + source_pattern + '\\b', str(line)):
                            command_list = self.__extract_subprocess(line)
                            if len(command_list):
    ...
    def auto_compile(self):
        ...
        if self.autogen:
            (status, output) = subprocess.getstatusoutput(
                "cd " + self.source_root + " && chmod u+x autogen.sh && ./autogen.sh")
            if status != 0:
                (status, output) = subprocess.getstatusoutput(
                    "cd " + self.source_root + " && dos2unix autogen.sh && ./autogen.sh")

            (status, output) = subprocess.getstatusoutput(
                "cd " + self.source_root + " && chmod u+x configure && ./configure")
            if status != 0:
                (status, output) = subprocess.getstatusoutput(
                    "cd " + self.source_root + " && dos2unix configure && ./configure")

            (status, output) = subprocess.getstatusoutput(
                "cd " + self.source_root + " && make VERBOSE=1 V=1 > __info_make")
            if status != 0:
            ...
            if os.path.exists(os.path.join(self.source_root, "configure")):
                ...
                (status, output) = subprocess.getstatusoutput(
                    "cd " + self.source_root + " && chmod u+x config && ./config")
                if status != 0:
                    (status, output) = subprocess.getstatusoutput(
                        "cd " + self.source_root + " && dos2unix config && ./config")
            (status, output) = subprocess.getstatusoutput(
                "cd " + self.source_root + " && make VERBOSE=1 V=1 > __info_make")
            if status != 0:
            ...
            (status, output) = subprocess.getstatusoutput(
                "cd " + self.source_root + "/build_path && cmake --debug-output --build -Wno-dev -D CMAKE_BUILD_TYPE=Debug ..")
            if status != 0:
                raise Exception("cmake error in the project")

            (status, output) = subprocess.getstatusoutput(
                "cd " + self.source_root + "/build_path && make VERBOSE=1 V=1 > __info_cmake")
            if status != 0:
                raise Exception("cmake error in the project")
            ...
            (status, output) = subprocess.getstatusoutput(
                "cd " + self.source_root + " && make VERBOSE=1 V=1 -f " + self.custom_makefile + " > __info_make")
            if status != 0:
    ...
    def auto_make_n(self):
        ...
        if self.autogen:
            ...
            if status != 0:
                (status, output) = subprocess.getstatusoutput(
                    "cd " + self.source_root + " && dos2unix configure && ./configure")
            (status, output) = subprocess.getstatusoutput("cd " + self.source_root + " && make -n > __info_make_n")
            if status != 0:
                raise Exception("autogen error in the project")
            ...
            (status, output) = subprocess.getstatusoutput(
                "cd " + self.source_root + "/build_path && make -n > __info_make_n")
            if status != 0:
                raise Exception("cmake error in the project")
            ...
            (status, output) = subprocess.getstatusoutput(
                "cd " + self.source_root + " && make -n -f " + self.custom_makefile + " > __info_make_n")
            if status != 0:
                raise Exception("custom_make error in the project")
            return self.source_root + "/__info_make_n"
        else:
    ...
    def get_make_info(self, force_make=False):
        ...
        if not force_make and os.path.exists(all_make_info_filename):
            ...
            if makeresult_path == "":
                ...
                bin_src_map, need_to_compile_files, status, files_compile_dir = get_file_dependency.get_need_to_compile_files(
                    makeresult_path, self.source_root, is_make_n=is_make_n)
